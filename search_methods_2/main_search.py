...
class PersonSearchEngine:
    ...
    def search_duckduckgo(self, query: str, max_results: int = 30) -> List[Dict]:
        """Search using DuckDuckGo with fallback to direct URL construction."""
        results = []
        try:
            # Try the HTML interface first
            url = f"https://html.duckduckgo.com/html/?q={quote(query)}"
            response = requests.get(url, headers=self.headers, timeout=10)
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Find result links - updated selectors for current DuckDuckGo HTML
            for result in soup.find_all('div', class_=['result', 'results_links_deep'])[:max_results]:
                link_elem = result.find('a', class_=['result__a', 'result__url'])
                if not link_elem:
                ...
                if link_elem:
                    ...
                    # Ensure URL is valid
                    if url and url.startswith(('http://', 'https://')) and url not in self.unique_urls:
            ...
            # If no results found with HTML parsing, fall back to basic search
            if not results:
                logger.info("DuckDuckGo HTML parsing failed, using fallback method")
                # As a fallback, we'll just return empty results rather than trying other methods
    ...
    def extract_contact_info(self, text: str) -> Dict[str, List[str]]:
        ...
        contact_info['phones'] = list(all_phones)
        ...
        contact_info['addresses'] = list(set(addresses))  # Remove duplicates
    ...
    def _save_results(self, all_results: List[Dict], social_results: Dict, analyzed_results: List[Dict]):
        ...
        # Save analyzed results - convert sets to lists for JSON serialization
        serializable_results = []
        for result in analyzed_results:
            result_copy = result.copy()
            if 'contact_info' in result_copy:
                # Already converted to lists in extract_contact_info
                pass
    ...
    def _generate_report(self, all_results: List[Dict], social_results: Dict, analyzed_results: List[Dict]):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if all_emails:
                f.write(f"\nEmails:\n")
                for email in sorted(all_emails):  # Sort for consistent output
                    f.write(f"  - {email}\n")
                    
            if all_phones:
                f.write(f"\nPhone Numbers:\n")
                for phone in sorted(all_phones):  # Sort for consistent output
                    f.write(f"  - {phone}\n")
                    
            if all_addresses:
                f.write(f"\nPossible Addresses:\n")
                for address in sorted(all_addresses):  # Sort for consistent output
                    f.write(f"  - {address}\n")
