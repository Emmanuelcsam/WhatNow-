"""
event_orchestrator.py - Main orchestrator that coordinates all services with free event discovery
"""
import asyncio
import logging
import time
...
class EventOrchestrator:
    ...
    def __init__(self, config, cache_service: CacheService):
        ...
        # Use free event discovery service instead of EventBrite
        self.event_service = FreeEventDiscoveryService(config)
        ...
        try:
            ...
            if not scraper_results:
                await self._update_status(session_id, 'processing', 30, 
                                        'Limited information found. Searching for events based on your activity description...')
                # Continue with just the activity description
                scraper_results = []
            ...
            # If no interests found from scraping, use activity-based interests
            if not processed_data.interests:
                logger.info("No interests extracted from scraping, using activity description")
                # Create interests from activity description
                activity_words = search_query['activity'].split()
            ...
            if self.config.ai.enable_ai and self.ai_integration.is_available():
                ...
                try:
                    ...
                    enhanced_interests = self.ai_integration.enhance_interests(
                        [{'keyword': i.keyword if hasattr(i, 'keyword') else i.get('keyword', ''), 
                          'category': i.category if hasattr(i, 'category') else i.get('category', 'general')} 
            ...
            await self._update_status(session_id, 'event_search', 70, 
                                    'Searching for relevant events from free sources...')
            ...
            # Step 6: Search events using free sources
            events = await self.event_service.search_events_async(
                keywords=keywords,
                location_data=search_query['location'],
                interests=[{
                    'keyword': i.keyword if hasattr(i, 'keyword') else i.get('keyword', ''),
                    'category': i.category if hasattr(i, 'category') else i.get('category', 'general'),
                    'confidence': i.confidence if hasattr(i, 'confidence') else i.get('confidence', 0.5)
                } for i in processed_data.interests],
                radius_miles=self.config.events.search_radius_miles,
                time_window_hours=self.config.events.time_window_hours
            )
            ...
            formatted_events = self.event_service.format_events_for_display(events)
            ...
            session_data = {
                    ...
                    'interests': [
                        {
                            ...
                            'category': i.category if hasattr(i, 'category') else i.get('category', 'general'),
                            'confidence': i.confidence if hasattr(i, 'confidence') else i.get('confidence', 0.5),
                            'sources': i.sources if hasattr(i, 'sources') else i.get('sources', [])
                ...
                'search_summary': {
                    ...
                    'event_sources': self.config.get_enabled_event_sources()
                },
                'step': 'complete'
            }
    ...
    def process_search_sync(self, search_query: Dict) -> Dict:
        ...
        try:
            ...
            # Format results
            return {
                'events': self.event_service.format_events_for_display(events),
                'processed_data': {
    ...
    def get_sample_events(self, location_data: Dict) -> List[Dict]:
        ...
        try:
            ...
            return self.event_service.format_events_for_display(events)
